...
#endif
{
    ...
    */
    ...
    pIe = wlan_hdd_get_vendor_oui_ie_ptr(WMM_OUI_TYPE, WMM_OUI_TYPE_SIZE,
                                         pBeacon->tail, pBeacon->tail_len);
    if (!pIe && (pConfig->SapHw_mode == eSAP_DOT11_MODE_11a ||
    ...
    if ((eSAP_DOT11_MODE_11ac == pConfig->SapHw_mode) ||
        (eSAP_DOT11_MODE_11ac_ONLY == pConfig->SapHw_mode)) {
                                         ...
                                         )
{
    ...
    if ((pHddStaCtx->conn_info.connState == eConnectionState_Associated) ||
        ...
        hddLog(LOGE, "%s: unexpected cfg disconnect called while in state (%d)",
